...
module.exports.wrapper = function Wrapper(key, secret, protocol, host, port, apiVersion, body, debug, type)
{
    ...
    self.connect = function(method, action, id, body, callback){
		...
		var options = { url: "http://" + self.host + "/" + action, headers: headers, method: method};
    ...
    function get_authorisation_http_header(method, action){
		...
		var hash, hmac;
        timestamp = 1396796334;//new Date().getTime();
        
		var nonce = "677df00e0a565f4bf7fe04c6805b77a6"; //stringy.random();
		var raw =  "";
		var raw = timestamp + "\n" + nonce + "\n" + method + "\n" + action + "\n" + self.host + "\n" + self.port +"\n";
